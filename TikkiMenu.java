...
/**
 * Dibujo del personaje Tikki con eventos (Menu).
 * 
 * @author Brandon Alexis Prado Castro (19051178)
 * @version 2.0. 20.09.2021
 */
public class TikkiMenu extends Application {
    @Override
    public void start(Stage primaryStage) {
        ...
        MenuItem item1 = new MenuItem("Mostrar Tikki original");
        MenuItem item2 = new MenuItem("Ocultar lunar a Tikki original");
        MenuItem item3 = new MenuItem("Cambiar color a morado");
        MenuItem item4 = new MenuItem("Cambiar color a amarillo");
        MenuItem item5 = new MenuItem("Cambiar color a verde");
        MenuItem item6 = new MenuItem("Salir");
        //Agrega los elementos al menu principal
        menu.getItems().addAll(item1, item2, item3, item4, item5, item6);
        //Se aplica una barra horizontal en la parte superior donde se encuentra el menu principal
        menuBar.prefWidthProperty().bind(primaryStage.widthProperty());
        //Se agregra toda la barra de menu a la ventana
        root.getChildren().add(menuBar);
        //Acciones de los items
        item1.setOnAction(event ->{
            draw(gc, Color.rgb(187, 10, 66));//Color original
        });
        item2.setOnAction(event ->{
           drawSinMancha(gc);
        });
        item3.setOnAction(event ->{
            draw(gc, Color.rgb(25, 41, 225));
        });
        item4.setOnAction(event ->{
            draw(gc, Color.rgb(200, 181, 26));
        });
        item5.setOnAction(event ->{
            draw(gc, Color.rgb(87, 166, 101));
        });
    ...
    private void drawSinMancha(GraphicsContext gc){
        //Llamar al dibujo original y se le envia como parametro el color original
        //todo esto para ahorrar codigo
        draw(gc, Color.rgb(187, 10, 66));
        //Una vez que se muestre el dibujo original
        //ocultamos la mancha superior creando otra misma del color de la cabeza
        gc.setLineWidth(3);
        gc.setStroke(Color.rgb(187, 10, 66));
        gc.setFill(Color.rgb(187, 10, 66));
        gc.beginPath();
        gc.appendSVGPath("M223,102 C223,102 228,99 236,95"
                + "C236,95 246,92 256,92"
