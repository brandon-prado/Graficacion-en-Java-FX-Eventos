...
/**
 * Dibujo del personaje Duusu con eventos (RadioButton).
 * 
 * @author Brandon Alexis Prado Castro (19051178)
 * @version 2.0. 20.09.2021
 */
public class DuusuRadioButton extends Application {
    @Override
    public void start(Stage primaryStage) {
        ...
        primaryStage.show();
        //RadioButton
        RadioButton rbt1 = new RadioButton("Mostrar Duusu original");
        RadioButton rbt2 = new RadioButton("Ocultar dibujo");
        RadioButton rbt3 = new RadioButton("Cambiar color a dorado");
        RadioButton rbt4 = new RadioButton("Cambiar color a naranja");
        RadioButton rbt5 = new RadioButton("Cambiar color a morado");
        RadioButton rbt6 = new RadioButton("Salir");
        //Default esta oculto
        rbt2.setSelected(true);
        //Agregar individualmente el RadioButton a la ventana
        root.getChildren().add(rbt1);
        root.getChildren().add(rbt2);
        root.getChildren().add(rbt3);
        root.getChildren().add(rbt4);
        root.getChildren().add(rbt5);
        root.getChildren().add(rbt6);
        //Ubicacion de cada RadioButton en la ventana
        rbt1.resizeRelocate(10,10,10,10);
        rbt2.resizeRelocate(10,40,10,10);
        rbt3.resizeRelocate(10,70,10,10);
        rbt4.resizeRelocate(10,100,10,10);
        rbt5.resizeRelocate(10,130,10,10);
        rbt6.resizeRelocate(10,160,10,10);
        //Acciones de los 6 RadioButton
        rbt1.setOnAction((ActionEvent event) -> {
            draw(gc, Color.rgb(26, 113, 211));
        });
        rbt2.setOnAction((ActionEvent event) -> {
            ocultar(gc);
        });
        rbt3.setOnAction((ActionEvent event) -> {
            draw(gc, Color.rgb(241, 196, 15));
        });
        rbt4.setOnAction((ActionEvent event) -> {
            draw(gc, Color.rgb(220, 118, 51));
        });
        rbt5.setOnAction((ActionEvent event) -> {
            draw(gc, Color.rgb(108, 52, 131));
        });
        rbt6.setOnAction((ActionEvent event) -> {
            System.exit(0);
        });
        
    }
     private void draw(GraphicsContext gc, Color color){   
        //CABEZA
        gc.setLineWidth(1);
    ...
    private void ocultar(GraphicsContext gc){
        //Ocultamiento con un rectangulo
        gc.setFill(Color.rgb(36, 211, 179));
