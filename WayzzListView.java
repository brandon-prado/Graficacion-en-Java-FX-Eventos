...
/**
 * Dibujo del personaje Wayzz con eventos (ListView).
 * 
 * @author Brandon Alexis Prado Castro (19051178)
 * @version 2.0. 20.09.2021
 */
public class WayzzListView extends Application {
    @Override
    public void start (Stage primaryStage){
        ...
        lv.getItems().addAll("Mostrar Wayzz original", "Mostrar sin antena", "Cambiar color a marron", "Cambiar color a verde fuerte", "Cambiar color a azul oscuro", "Salir");
        //Tamaño maximo de la lista
        lv.setMaxSize(170, 140);
        root.getChildren().add(lv);
        //Acciones del ListView
        lv.setOnMouseClicked(event ->{
            if(lv.getSelectionModel().getSelectedItem().equals("Mostrar Wayzz original")){
                draw(gc, Color.rgb(68, 189, 94));
            }else if(lv.getSelectionModel().getSelectedItem().equals("Mostrar sin antena")){
                drawSinAntena(gc);
            }else if(lv.getSelectionModel().getSelectedItem().equals("Cambiar color a marron")){
                draw(gc, Color.rgb(198, 104, 50));
            }else if(lv.getSelectionModel().getSelectedItem().equals("Cambiar color a verde fuerte")){
                draw(gc, Color.rgb(50, 110, 68));
            }else if(lv.getSelectionModel().getSelectedItem().equals("Cambiar color a azul oscuro")){
                draw(gc, Color.rgb(13, 31, 38));
    ...
    private void drawSinAntena(GraphicsContext gc){
        //Llamar al dibujo original y se le envia como parametro el color original
        //todo esto para ahorrar codigo
        draw(gc, Color.rgb(68, 189, 94));
        //Ocultamiento de la antena superior
        gc.setLineWidth(3);
        gc.setStroke(Color.rgb(224, 151, 115));
        gc.setFill(Color.rgb(224, 151, 115));
        gc.beginPath();
        gc.appendSVGPath("M237,87 C237,87 239,77 242,66"
                + "C242,66 245,57 251,40"
                + "C251,40 247,37 246,31"
        ...
        gc.setStroke(Color.BLACK);
        gc.beginPath();
        gc.appendSVGPath("M235,87 C235,87 250,87 250,87");  
        gc.closePath();
        gc.fill();
        gc.stroke();
    }
